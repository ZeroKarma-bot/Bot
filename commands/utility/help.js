...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const embed = new EmbedBuilder()
      .setColor(embedColor)
      .setTitle('Help Menu')
      .setDescription('Select a category from the dropdown menu below to view the commands available:')
      .setFooter({ text: 'Use the dropdown menu to navigate through the command categories.' })
      .setTimestamp();
      ...
      switch (i.values[0]) {
        case 'ecosystem':
          categoryEmbed = new EmbedBuilder()
            ...
            .addFields(
              { name: '**/karma check**', value: 'Check your experience and level' },
              { name: '**/karma daily**', value: 'Claim Daily Karma' },
              { name: '**/goodmorning** `[User]`', value: 'Send a good morning message and earn karma' },
              { name: '**/match** `[User 1]` `[User 2]`', value: 'Check compatibility between two users. If you get 90% or over, you get a huge amount of karma experience' },
              { name: '**/welcome** `[User]`', value: 'Welcome new users and get a karma reward' },
              { name: '**/goodnight** `[User]`', value: 'Send a good night message and earn karma' }
            );
          break;
        case 'fun':
          categoryEmbed = new EmbedBuilder()
            ...
            .addFields(
              ...
              { name: '**/akinator**', value: 'Play a guessing game with Akinator' },
        ...
        case 'admin':
          categoryEmbed = new EmbedBuilder()
            ...
            .addFields(
              { name: '**/togglecommand** `[Command Name]` `[Enable/Disable]`', value: 'Enables or disables a command on the bot in the server. Only administrators can use this command.' }
            );
          break;
      }
    ...
    collector.on('end', collected => {
      if (collected.size === 0) {
        interaction.editReply({ content: 'No category selected.', components: [] });
      }
    });
  },
};
